...
CREATE INDEX IF NOT EXISTS arr_dep_query_id_idx ON arr_dep (query_id);`
...
const createStorage = (db) => {
	...
	const readJourneys = (inputHash, createdMin, createdMax) => {
		debug('readJourneys', {inputHash, createdMin, createdMax})
		return readAtomic('journeys', inputHash, createdMin, createdMax)
	}
	const writeJourneys = async (inputHash, created, journeys) => {
		debug('writeJourneys', {inputHash, created, journeys})
		return writeAtomic('journeys', inputHash, created, journeys)
	}
